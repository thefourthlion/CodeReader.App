...
function useListboxItem(originalProps) {
  ...
  const FragmentWrapper = href ? "a" : import_react.Fragment;
...
var VirtualizedListbox = (props) => {
  ...
  if (!virtualization || !(0, import_shared_utils3.isEmpty)(virtualization) && !virtualization.maxListboxHeight && !virtualization.itemHeight) {
    throw new Error(
